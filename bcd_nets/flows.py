...
def get_flow_CIF(rand_key, input_shape, num_layers, batch_size, threshold=-1_000, init_std=3.0, pretrain=True):
    ...
    def sample_flow(params: hk.Params, state: hk.State, rng_key, n: int) -> Tuple[jnp.ndarray, jnp.ndarray, hk.State]:
        ...
        sample_log_probs = jnp.clip(-out["log_det"] + logprob, a_min=threshold, a_max=None)
        return out["x"], sample_log_probs, state,
    ...
    if pretrain:
        ...
        def step(p, state, opt_p, key):
            ...
            (_, state), flow_grad = value_and_grad(loss_fn, has_aux=True)(p, state)
