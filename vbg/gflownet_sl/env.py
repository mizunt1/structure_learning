...
from vbg.gflownet_sl.utils.cache import LRUCache
...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    def __init__(
        ...
        self.variables = scorer.data.columns.tolist()
    ...
    def step(self, actions):
        ...
        # The environment returns log R(G_t) - log R(G_{t-1}), corresponding to
        # a local change in the scores. This quantity can be used directly in
        # the loss function derived from the trajectory detailed loss.
        if self.vb == True:
            diff_log_rewards = np.zeros(len(actions))
        else:
            diff_log_rewards = self.local_scores_wait(keys, local_cache, data)

        # Update the scores
        self._state['score'] += diff_log_rewards
        self._state['score'][dones] = 0

        return (deepcopy(self._state), diff_log_rewards, dones, {})
    ...
    def local_scores_wait(self, keys, local_cache, data):
        ...
        if self.num_workers > 0:
            ...
            for target, indices, indices_after in data:
                ...
                self.local_scores[local_score_after.key] = local_score_after.score
                if local_score_before is not None:
                    self.local_scores[local_score_before.key] = local_score_before.score

        diff_local_scores = []
        for target, key_tm1, key_t in keys:
            if target is None:
                diff_local_scores.append(0.)

            else:
                diff_local_scores.append(
                    self.local_scores[(target, key_t)]
                    - self.local_scores[(target, key_tm1)]
                )

        return np.array(diff_local_scores, dtype=np.float_)
...
if __name__ == '__main__':
    ...
    from gflownet_sl.scores.bde_score import BDeuScore

    rng = default_rng(0)

    model = get_example_model('sachs')
    samples = sample_from_discrete(model, num_samples=1000)

    env = GFlowNetDAGEnv(
        num_envs=32,
        scorer=BDeuScore(data=samples),
        max_parents=2,
        num_workers=4,
        context='spawn'
    )
    ...
    for _ in range(10):
        ...
        actions = get_random_actions(observations['mask'], rng, weight=1)
        observations, rewards, dones, _ = env.step(actions)
        print(rewards.shape)  # The difference in log-rewards: log R(G_t) - log R(G_{t-1})

    print(f'Size of the cache: {len(env.local_scores)}')
    env.close()
